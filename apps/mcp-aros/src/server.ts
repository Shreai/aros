...
const operatorTools: McpTool[] = [
  ...
  },
  {
    ...
    description: 'Read low-stock, fast-moving, stale, and reorder-risk inventory signals.',
    inputSchema: objectSchema({
      storeIds: arraySchema('string', 'AROS store IDs to include.'),
      horizonDays: numberSchema('Forecast horizon in days.'),
  ...
  },
  {
    ...
    description: 'Read cashier, refund, void, no-sale, and operational exception summaries.',
    inputSchema: objectSchema({
      storeIds: arraySchema('string', 'AROS store IDs to include.'),
      startDate: stringSchema('Start date in YYYY-MM-DD format.'),
      endDate: stringSchema('End date in YYYY-MM-DD format.'),
      types: arraySchema('string', 'Exception types to include.')
    }, ['storeIds', 'startDate', 'endDate']),
    annotations: { readOnlyHint: true, destructiveHint: false }
  },
  {
...
const toolsBySurface: Record<Surface, McpTool[]> = {
...
async function callTool(params: ToolCallParams, req: IncomingMessage, surface: Surface) {
  ...
  if (name === 'aros_get_store_summary') {
    ...
    return toolText(await arosFetch(`/api/store/summary?${query}`, operatorAuthHeader, correlationId, {}, authContext));
  }

  if (name === 'aros_get_connector_health') {
    return toolText(await arosFetch('/api/connectors', operatorAuthHeader, correlationId, {}, authContext));
  }

  if (name === 'aros_draft_action') {
    return toolText(await arosFetch('/api/human/tasks', operatorAuthHeader, correlationId, {
      method: 'POST',
      body: JSON.stringify({
        title: args.title,
...
function demoResult(name: string, args: Record<string, unknown>, correlationId: string, surface: Surface) {
...
function addQuery(query: URLSearchParams, key: string, value: unknown) {
