export type StoredTransaction = { verifier: string; nonce: string; returnTo: string; requestedExperience?: 'aros' | 'mib'; expiresAt: number };
export type StoredSession = { subject: string; userId?: string; workspaceId: string; role: string; claims: Record<string, unknown>; refreshToken?: string; expiresAt: number };
...
export function createSupabaseOidcStore(db: SupabaseLike, crypto: { seal(value: string): string; open(value: string): string }): OidcStore {
  ...
  return {
    ...
    async putSession(hash, value) { const { error } = await db.from('oidc_rp_sessions').insert({ session_hash: hash, subject: value.subject, user_id: value.userId ?? null, workspace_id: value.workspaceId, role: value.role, sealed_payload: crypto.seal(JSON.stringify({ claims: value.claims, refreshToken: value.refreshToken })), expires_at: new Date(value.expiresAt).toISOString() }); if (error) throw new Error(`OIDC session persistence failed: ${error.message}`); },
    async getSession(hash, now) { const { data, error } = await db.from('oidc_rp_sessions').select('subject,user_id,workspace_id,role,sealed_payload,expires_at').eq('session_hash', hash).is('revoked_at', null).gt('expires_at', new Date(now).toISOString()).maybeSingle(); if (error) throw new Error(`OIDC session lookup failed: ${error.message}`); if (!data) return null; const secret = decode<{ claims: Record<string, unknown>; refreshToken?: string }>(data.sealed_payload); return { subject: data.subject, userId: data.user_id ?? undefined, workspaceId: data.workspace_id, role: data.role, expiresAt: new Date(data.expires_at).getTime(), ...secret }; },
    async revokeSession(hash) { const { data: existing, error: loadError } = await db.from('oidc_rp_sessions').select('subject,user_id,workspace_id,role,sealed_payload,expires_at').eq('session_hash', hash).is('revoked_at', null).maybeSingle(); if (loadError) throw new Error(`OIDC session revoke lookup failed: ${loadError.message}`); if (!existing) return null; const { error } = await db.from('oidc_rp_sessions').update({ revoked_at: new Date().toISOString() }).eq('session_hash', hash).is('revoked_at', null); if (error) throw new Error(`OIDC session revoke failed: ${error.message}`); const secret = decode<{ claims: Record<string, unknown>; refreshToken?: string }>(existing.sealed_payload); return { subject: existing.subject, userId: existing.user_id ?? undefined, workspaceId: existing.workspace_id, role: existing.role, expiresAt: new Date(existing.expires_at).getTime(), ...secret }; },
